...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            newCorners = state[1][0:4]
            
            if not hitsWall:
                for corner in self.corners:
                    if nextPosition == corner:
                        newCorners[self.corners.index(corner)] = True
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for corner in corners:
        if not food[corners.index(corner)]:
    ...
    # for corner in corners:
    #     if not food[corners.index(corner)]:
    #         dist = ((position[0]-corner[0])**2 + (position[1]-corner[1])**2)**0.5
    #         if dist> cost:
    #             cost = dist
    # return cost
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for food in foodList:
        if foodGrid[food[0]][food[1]]:
            left = min(position[0], maxPosition[0])
            right = max(position[0], maxPosition[0])
            top = max(position[1], maxPosition[1])
            bottom = min(position[1], maxPosition[1])
            if not (left <= food[0] and food[0] <= right and bottom <= food[1] and food[1] <= top):
                cost += 1 
